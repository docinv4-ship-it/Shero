...
function BrowsePageContent() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const keyword =
        baseQuery ||
        catKeywords[((page - 1) + 1) % (catKeywords.length || 1)] ||
        BROWSE_KEYWORDS[((page - 1) + 1) % BROWSE_KEYWORDS.length];

      const params = new URLSearchParams({
        q: keyword,
        page: String(page),
        pageSize: "50",
        seed: String(page * 997),
      });
        ...
        <div className="flex items-center justify-between mb-4 gap-2">
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white hover:border-orange-300 transition-colors"
            >
